...
def extract_feature(model, data_loader, device, opt):
    model.eval()
    ...
    for batch_idx, batch in tqdm(enumerate(data_loader), mininterval=2,
                                 desc="  Extracting =>", total=len(data_loader)):
        with torch.no_grad():
            # prepare data
            batched_data = [prepare_batch_inputs(step_data, device=device, non_blocking=opt.pin_memory)
                            for step_data in batch[0]] # batch[0]

            # TODO: 必ず以下のlen(x)が11になる. 11とは何?を調べる -> 11はmax sequence
            ...
            # pos
            pos_step_id_list = [e.cuda() for e in batch[4]]
            ...
            # forward & backward
            video_vectors, ingr_vectors = model(input_ids_list, video_features_list,
                                                input_masks_list, token_type_ids_list, 
                                                input_labels_list, ingr_input_ids,
                                                ingr_masks, ingr_sep_masks, batch_step_num,
                                                ingr_id_dict, extra_zeros, pos_step_id_list,
                                                predict=True, is_gumbel=True)

            all_video_vectors += video_vectors
            all_ingr_vectors += ingr_vectors
            all_infos += batch[2]

    result_dicts = []
    for video_vectors, ingr_vectors, infos in zip(all_video_vectors, all_ingr_vectors, all_infos):
        result_dicts.append({
            "video_feat" : video_vectors.cpu().detach(),
            "ingredient_feat" : ingr_vectors.cpu().detach(),
            "info" : infos,
            })
    return result_dicts
...
def get_args():
    ...
    parser.add_argument("--pos_vocab_word2idx_path", type=str, default="./cache/pos_vocab_word2idx.json")
...
def save(results, mode):
    data_path = os.path.join("/mnt/LSTA5/data/nishimura/graph_youcook2_generator/proposed_method/extracted_features", mode)
    for result in results:
        recipe_id = result["info"]["name"]
        filename = os.path.join(data_path, recipe_id + ".pkl")
...
def main():
    ...
    train_dataset = RCDataset(
        ...
        word2idx_path=opt.word2idx_path, pos_word2idx_path=opt.pos_vocab_word2idx_path, max_t_len=opt.max_t_len,
    ...
    rt_config = EDict(
        ...
        pos_vocab_size=train_dataset.pos_vocab_size, # pos_vocab_size
    ...
    #checkpoint = torch.load(os.path.join(opt.save_model, "best_model.ckpt"))
    checkpoint = torch.load("/mnt/LSTA5/data/nishimura/graph_youcook2_generator/proposed_method/pretrain_captioning/ingr.chkpt")
    model.load_state_dict(checkpoint["model"])
    model.cuda()
    model.eval()
    print("loaded!")

    train_results = extract_feature(model, train_loader, device, opt)
    val_results = extract_feature(model, val_loader, device, opt)

    save(train_results, "training")
    save(val_results, "validation")
