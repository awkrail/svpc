...
def save_bosselut_verb_vocab(path="./bosselut_verb_vocab.json"):
    with open(path, "r") as f:
        action_dict = json.load(f)

    with open("cache/bosselut_verb_vocab.json", "w") as f:
        json.dump(action_dict, f)

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--train_path", type=str, required=True)
    parser.add_argument("--dset_name", type=str, default="yc2")
    ...
    print("[Info] Copy bosselut verb vocab to cache/")
    save_bosselut_verb_vocab()
    print("[Info] Finish.")
    ...
    print("[Info] Extracting glove embeddings w/ verb vocabulary")
    verb2idx = load_json("cache/bosselut_verb_vocab.json")
    verb_vocab_glove_path = os.path.join(opt.cache, "bosselut_{}_verb_vocab_glove.pt".format(opt.dset_name))
    extract_glove(verb2idx, opt.raw_glove_path, verb_vocab_glove_path)
    print("[Info] Finish.")
