...
model_path="/mnt/LSTA5/data/nishimura/graph_youcook2_generator/proposed_method/pretrain_captioning/${model_type}"
...
echo "---------------------------------------------------------"
extra_args=()
if [[ ${model_type} == "full" ]]; then  # w/ refinements
    extra_args+=(--recurrent)
    extra_args+=(--ours)
    extra_args+=(--full)
elif [[ ${model_type} == "wo_refinements" ]]; then   # w/o refinements
    extra_args+=(--recurrent)
    extra_args+=(--ours)
    extra_args+=(--wo_refinements)
elif [[ ${model_type} == "copy" ]]; then    # w/ only copy
    extra_args+=(--recurrent)
    extra_args+=(--ours)
    extra_args+=(--copy)
elif [[ ${model_type} == "struct" ]]; then # w/ only structure
    extra_args+=(--recurrent)
    extra_args+=(--ours)
    extra_args+=(--struct)
